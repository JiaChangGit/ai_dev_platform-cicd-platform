...
class GenerateSbomTest(unittest.TestCase):
    def test_generates_spdx_for_packaged_files(self):
        with tempfile.TemporaryDirectory() as temp:
            archive_path = Path(temp) / "ai-dev-platform-1.5.0.zip"
            ...
            self.assertEqual([item["name"] for item in result["packages"]], ["ai-dev-platform"])
            self.assertIn("/releases/tag/v1.5.0/spdx-", result["documentNamespace"])
            self.assertFalse(
