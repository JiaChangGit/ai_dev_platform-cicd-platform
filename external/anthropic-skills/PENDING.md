...
這個目錄是 `anthropic-skills` subtree 的預留位置，目前尚未執行同步。

1. 到你自己的 GitHub 帳號 fork 對應的第三方倉庫
2. 編輯 `../subtrees.yaml`，把 `anthropic-skills` 項目的 `repo` 換成你的 fork URL
3. Commit 這個修改（git subtree 要求工作目錄乾淨才能執行，這步不能省）：

   ```bash
   git add external/subtrees.yaml
   git commit -m "chore(sync): point anthropic-skills at fork"
   ```
