...
#
# scripts/sync.sh
# 依 external/subtrees.yaml 的設定，透過 git subtree 同步第三方儲存庫。
#
# 用法:
...
#
# 注意: add/pull 執行前會先檢查是否在 git 儲存庫內、且工作目錄乾淨——這是
...
# git subtree add/pull 要求：(1) 必須在 git 儲存庫內執行 (2) 工作目錄必須乾淨（無
...
if [ "$cmd" = "add" ] || [ "$cmd" = "pull" ]; then
  if ! git -C "$ROOT_DIR" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    echo "[錯誤] $ROOT_DIR 不是一個 git 儲存庫。" >&2
    echo "       add/pull 只供 ai_dev_platform-cicd-platform 維護儲存庫使用。" >&2
    echo "       若這是下載後的唯讀 ai-dev-platform，請勿 git init 或同步 subtree。" >&2
    exit 1
  fi
      ...
      if [ -d "$prefix" ]; then
        ...
        if [ -n "$contents" ]; then
          if [ "$contents" = "PENDING.md" ]; then
            # 這是本儲存庫自己放的佔位檔（見 external/README.md），代表尚未同步，
