...
`ai-dev-platform` 的其餘部分刻意保持「產品無關」，同一套 workflow/governance 要能同時適用 Android App 開發與 Linux kernel 開發。但實際寫程式碼時，AI 一定會需要該領域的具體知識（例如 Android 的生命週期慣例、kernel 的鎖策略）。

這份文件**不提供**這些具體知識本身，而是告訴 AI 代理人：遇到領域相關問題時，該去哪裡查、優先信任哪些來源。查到的結果應該整理進**產品倉庫**自己的 `docs/domain-standards.md`，附上來源連結，不要回寫進 `ai-dev-platform`。
...
第一次 bootstrap 產品倉庫時查到的資料，不代表之後就不用再查。`product-cicd-platform/docs/domain-standards.md` 應該被當成活文件持續維護：
...
- 若查證後真的找不到來源（會員制規範、團隊內規、需要人類裁定的商業決策），不要用猜的填空，見 `governance/agent-discipline.md` 2.3 節「查不到來源的時候」
...
2. 找該領域最活躍的原始碼倉庫，觀察其實際慣例（但需註明這是「觀察慣例」而非「官方規定」）
3. 把找到的來源與整理結果記錄進產品倉庫的 `docs/domain-standards.md`，並在 `AGENTS.md`（產品倉庫自己的版本，若有）中補上指向該文件的提示
